            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    for index in range(len(severity)):
                        ...
                        device=device_id
                        if sev=="1":
                            print(f"Detected potential catastrophic bug in {device}: {bug}")
        ...
        if version:
            try:
                ...
                if response.status_code == 200:
                    ...
                    advisory_id2 = [ adv['advisoryId'] for adv in response_data['advisories'] if adv['cvssBaseScore'] > "8.9" ]
                    Summary = [ adv['summary'] for adv in response_data['advisories'] if adv['cvssBaseScore'] > "8.9" ]
                    
                    # Update the bug_id information in the proper column/row
                    for index in range(len(advisory_id2)):
                        ad_id2=advisory_id2[index]
                        Summ=Summary[index]
                        print(f"PSIRT critical for {version}: {ad_id2}, se muestra resumen a continuacion: {Summ}")
    ...
    for index, row in df.iterrows():
        ...
        if row['OS type']== 'IOS-XE' and row['Configured restconf'] == "No Restconf":
            ...
            try:
                ...
                print(f"Failed to retrieve info from {ip_address}: {str(e)}")
        elif row['OS type']== 'IOS-XE' and row['Configured restconf'] == "Restconf enabled":
            device = {
                'ip': ip_address,
                'username': 'admin',
                'password': 'cisco!123',
            ...
            if response.status_code == 200:
                ...
                for element in memory_statistics:
                    if element['name'] == 'Processor':
                        ...
                        used_percentage_memory = (int(free_memory)/int(total_memory))*100
                        ...
                        if used_percentage_memory >=90:
                            print(f"El dispositivo excede el 90% de uso de memoria, lo que podria provocar lentitud en el sistema")
